...
import { useRunSummary } from "./lib/hooks";
...
export default function AnalysisDashboard() {
  ...
  const { summary, loading, error } = useRunSummary(selectedRunId);

  // Refresh the run picker every 20 seconds.
  useEffect(() => {
    let cancelled = false;
    ...
    fetchRuns();
    const id = setInterval(fetchRuns, 20_000);
